...
impl RistrettoPoint {
    ...
    /// Decode a point from compressed form.
...
impl RistrettoScalar {
    ...
    /// Construct a scalar by reducing a 64-byte little-endian integer modulo the group order.
    pub fn from_bytes_mod_order_wide(bytes: &[u8; 64]) -> Self {
        RistrettoScalar(ExternalRistrettoScalar::from_bytes_mod_order_wide(bytes))
    }

    /// Construct a scalar by reducing a 32-byte little-endian integer modulo the group order.
