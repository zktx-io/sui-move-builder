...
impl Secp256k1<Signing> {
    ...
    pub fn sign_ecdsa_recoverable(
        ...
        let (sig, recid) = signing_key
            .sign_prehash_recoverable(&msg.0)
    ...
    pub fn sign_schnorr(&self, msg: &Message, keypair: &KeyPair) -> schnorr::Signature {
        ...
        let sig: k256::schnorr::Signature =
            k256::schnorr::signature::Signer::sign(&signing_key, &msg.0);
...
impl<C> Secp256k1<C> {
    pub fn verify_schnorr(
        ...
        let vk =
            k256::schnorr::VerifyingKey::from_bytes(&pk.0).map_err(|_| Error::InvalidPublicKey)?;
        let k256_sig = k256::schnorr::Signature::try_from(sig.as_ref())
...
pub mod ecdsa {
    ...
    impl RecoverableSignature {
        pub fn from_compact(data: &[u8], recid: RecoveryId) -> Result<Self, super::Error> {
            let sig =
                K256Signature::try_from(data).map_err(|_| super::Error::IncorrectSignature)?;
            let rec = K256RecoveryId::try_from(recid.0 as u8)
                .map_err(|_| super::Error::InvalidRecoveryId)?;
    ...
    impl RecoveryId {
        ...
        pub fn from_i32(i: i32) -> Result<Self, super::Error> {
            if i < 0 || i > 3 {
                return Err(super::Error::InvalidRecoveryId);
            }
            Ok(RecoveryId(i))
        }
    }
...
pub mod schnorr {
    ...
    impl Signature {
        pub fn from_slice(data: &[u8]) -> Result<Self, super::Error> {
            if data.len() != 64 {
                return Err(super::Error::IncorrectSignature);
            }
            let mut arr = [0u8; 64];
            arr.copy_from_slice(data);
