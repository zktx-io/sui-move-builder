...
pub mod bn254 {
    pub mod poseidon {
        pub fn poseidon_bytes(_: &Vec<Vec<u8>>) -> Result<Vec<u8>, String> {
            Ok(vec![])
        }
    }
    pub mod api {
        ...
        pub fn verify_groth16_in_bytes(
            ...
            let gamma_abc_g1: Vec<G1Affine> =
                CanonicalDeserialize::deserialize_compressed(vk_gamma_abc_g1_bytes)
                    .map_err(|e| format!("Failed to deserialize gamma_abc_g1: {}", e))?;

            let alpha_g1_beta_g2: Fq12 =
                CanonicalDeserialize::deserialize_compressed(alpha_g1_beta_g2_bytes)
                    .map_err(|e| format!("Failed to deserialize alpha_g1_beta_g2: {}", e))?;

            let gamma_g2_neg_pc: <Bn254 as ark_ec::pairing::Pairing>::G2Prepared =
            ...
            let delta_g2_neg_pc: <Bn254 as ark_ec::pairing::Pairing>::G2Prepared =
            ...
            let proof: Proof<Bn254> =
                CanonicalDeserialize::deserialize_compressed(proof_points_bytes)
                    .map_err(|e| format!("Failed to deserialize proof: {}", e))?;

            let public_inputs: Vec<ark_bn254::Fr> =
                CanonicalDeserialize::deserialize_compressed(proof_inputs_bytes)
    ...
    pub mod zk_login {
        ...
        impl OIDCProvider {
            pub fn from_iss(_iss: &str) -> Result<Self, String> {
                Ok(OIDCProvider)
            }
        }
        ...
        impl ZkLoginInputs {
            ...
            pub fn init(&self) -> Result<(), FastCryptoError> {
                Ok(())
            }
            pub fn from_json<T: AsRef<[u8]>>(_s: &str, _seed: T) -> Result<Self, String> {
                Ok(ZkLoginInputs)
            }
        }
    }
...
pub mod bls12381 {
    pub struct Fr;
    pub mod api {
        pub fn prepare_pvk_bytes(_: &[u8]) -> Result<Vec<Vec<u8>>, String> {
            Ok(vec![])
        }
        pub fn verify_groth16_in_bytes(
            _: &[u8],
