...
pub mod fs {
    ...
    pub fn fcntl_getfl(_: i32) -> Result<i32, i32> {
        Ok(0)
    }
    pub fn rename<P: AsRef<std::path::Path>, Q: AsRef<std::path::Path>>(
        _: P,
        _: Q,
    ) -> Result<(), super::io::Errno> {
        Ok(())
    }
    pub fn unlink<P: AsRef<std::path::Path>>(_: P) -> Result<(), super::io::Errno> {
        Ok(())
    }
    ...
    pub fn renameat_with<P: AsRef<std::path::Path>, Q: AsRef<std::path::Path>>(
        _: i32,
        _: P,
        _: i32,
        _: Q,
...
pub mod termios {
    ...
    pub fn isatty<T>(_fd: T) -> bool {
        false
    }
    pub fn tcgetwinsize<T>(_fd: T) -> Result<Winsize, i32> {
        Err(0)
    }
}
pub use backend::c::*;
