...
pub fn decode_all<R: std::io::Read>(_read: R) -> Result<Vec<u8>, std::io::Error> {
    Ok(vec![])
}
pub fn encode_all<R: std::io::Read>(_read: R, _level: i32) -> Result<Vec<u8>, std::io::Error> {
    Ok(vec![])
}
pub fn bulk_decompress(_src: &[u8], _dst: &mut [u8]) -> Result<usize, std::io::Error> {
    Ok(0)
}
pub mod stream {
    ...
    impl<'a, R: std::io::Read> Decoder<'a, R> {
        pub fn new(reader: R) -> Result<Self, std::io::Error> {
            Ok(Self(reader, std::marker::PhantomData))
        }
    }
