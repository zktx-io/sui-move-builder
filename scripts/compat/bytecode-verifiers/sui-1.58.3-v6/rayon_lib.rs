...
pub mod collections {
    pub mod btree_map {
        ...
        pub type IterMut<'a, K, V> =
            crate::StubIter<std::collections::btree_map::IterMut<'a, K, V>>;
    }
}
...
pub trait ParallelIterator: Sized {
    ...
    where
        T: ParallelIterator<Item = Self::Item>,
    {
    ...
    F: FnMut(I::Item) -> R,
{
        ...
        ID: FnMut() -> Self::Item,
    {
        ...
        self.base
            .try_fold(identity, |acc, item| Ok::<R, ()>(op(acc, map_op(item))))
            .unwrap()
    }
    ...
    where
        ID: FnMut() -> T,
        FoldOp: FnMut(T, Self::Item) -> Result<T, E>,
    {
        let mut map_op = self.map_op;
        self.base
            .try_fold(identity, |acc, item| fold_op(acc, map_op(item)))
    }

    fn for_each<OP>(self, mut op: OP)
    where
        OP: FnMut(Self::Item),
    {
        let mut map_op = self.map_op;
