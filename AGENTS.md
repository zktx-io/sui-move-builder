...
Legacy verification builds must stay isolated under `.sui-build/bytecode-verifiers/<verifier-id>/`. Do not use `.sui-build/legacy-investigation/`, old npm package contents, or old builder source as build inputs; those are analysis artifacts only. Legacy verifier packages are distributed as separate `@zktx.io/sui-move-builder-bytecode-verifier-<verifier-id>` packages and loaded by the main package lazily. The main package must not bundle legacy WASM artifacts. If a legacy build reports that it is falling back to the current compat overlay, treat that as a porting signal and add a verifier-specific `scripts/compat/bytecode-verifiers/<verifier-id>/manifest.json` before trusting later Cargo errors.
