...
| Source provenance verification  | `crates/sui-source-validation/src/lib.rs`, `crates/sui/src/client_commands.rs`                                                                       | `src/verification.ts`, `sui-move-wasm/src/verification.rs`                                                                | rust self-impl      | Verification provenance tests and expected-status artifact audits       | Re-check `CompiledModule` deserialization/equality, root address substitution, and status precedence. |
