...
fn visit_module_for_order(
    id: &ModuleId,
    module_indexes: &BTreeMap<ModuleId, usize>,
    module_infos: &[(ModuleId, move_compiler::compiled_unit::NamedCompiledModule)],
    marks: &mut BTreeMap<ModuleId, u8>,
    ordered: &mut Vec<ModuleId>,
) -> Result<(), String> {
    match marks.get(id).copied() {
        Some(2) => return Ok(()),
        Some(1) => return Err(format!("Circular dependency detected at {}", id)),
        _ => {}
    }

    marks.insert(id.clone(), 1);
    let index = module_indexes
        .get(id)
        .copied()
        .ok_or_else(|| format!("Unknown module in ordering: {}", id))?;
    for dep in module_infos[index].1.module.immediate_dependencies() {
        if module_indexes.contains_key(&dep) {
            visit_module_for_order(&dep, module_indexes, module_infos, marks, ordered)?;
        }
    }
...
fn root_module_order(
    module_infos: &[(ModuleId, move_compiler::compiled_unit::NamedCompiledModule)],
) -> Result<Vec<ModuleId>, String> {
    let module_indexes = module_infos
        .iter()
        .enumerate()
    ...
    for (id, _) in module_infos {
        visit_module_for_order(id, &module_indexes, module_infos, &mut marks, &mut ordered)?;
    }
    Ok(ordered)
}
...
pub(crate) fn compile_impl(
    ...
    match res {
        Ok((units, warning_diags)) => {
            ...
            let ordered_ids: Vec<ModuleId> = match root_module_order(&module_infos) {
                Ok(ids) => ids,
                Err(e) => {
                    return CompileResult {
                        success: false,
