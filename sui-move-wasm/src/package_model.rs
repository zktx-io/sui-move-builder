...
fn numerical_address(addr: &str) -> Option<NumericalAddress> {
    parse_hex_address_to_bytes(addr)
        .map(|bytes| NumericalAddress::new(bytes, move_compiler::shared::NumberFormat::Hex))
}
...
fn address_map_from_manifest(manifest: &SourceManifest) -> BTreeMap<String, NumericalAddress> {
...
fn root_package_snapshot(files: &BTreeMap<String, String>) -> RootPackageSnapshot {
    ...
    if let Some(manifest) = parse_source_manifest(files) {
        ...
        snapshot.named_address_map = address_map_from_manifest(&manifest);
    }
...
fn resolved_package_snapshot(
    ...
    if let Some(ref addr_map) = pkg_group.address_mapping {
        ...
        named_address_map = address_map_from_manifest(manifest);
