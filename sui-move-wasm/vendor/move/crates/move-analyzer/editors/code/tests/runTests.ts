...
async function runVSCodeTest(vscodeVersion: string): Promise<void> {
  try {
    ...
    // The workspace
    let testWorkspacePath = path.resolve(
      __dirname,
      "./lsp-demo/lsp-demo.code-workspace"
    );
    if (process.platform === "win32") {
      testWorkspacePath = path.resolve(
        __dirname,
        "./lsp-demo/lsp-demo-win.code-workspace"
      );
    }
    ...
    cp.spawnSync(
      newCli,
      [...args, "--install-extension", "damirka.move-syntax", "--force"],
      {
        encoding: "utf-8",
        stdio: "inherit",
      }
