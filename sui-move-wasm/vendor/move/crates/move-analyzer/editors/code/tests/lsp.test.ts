...
const isFunctionInCompletionItems = (
  fnName: string,
  ...
  return (
    items.find(
      (item) =>
        item.label === fnName && item.kind === CompletionItemKind.Function
    ) !== undefined
  );
};

const isKeywordInCompletionItems = (
  label: string,
  items: vscode.CompletionItem[]
): boolean => {
  return (
    items.find(
      (item) => item.label === label && item.kind === CompletionItemKind.Keyword
    ) !== undefined
  );
};
...
Mocha.suite("LSP", () => {
  Mocha.test("textDocument/documentSymbol", async () => {
    ...
    const docs = await vscode.workspace.openTextDocument(
      path.join(workDir, "sources/M1.move")
    );
    ...
    const syms: Array<lc.DocumentSymbol> | undefined =
      await vscode.commands.executeCommand(
        "move-analyzer.textDocumentDocumentSymbol",
        params
      );
    ...
    assert.deepStrictEqual(
      syms[0]?.children[1]?.children[0]?.kind,
      lc.SymbolKind.Field
    );
    assert.deepStrictEqual(
      syms[0]?.children[1]?.children[0]?.name,
      "some_field"
    );
    assert.deepStrictEqual(syms[0]?.children[1].name, "SomeOtherStruct");
    ...
    assert.deepStrictEqual(
      syms[0]?.children[3]?.detail,
      '["test", "expected_failure"]'
    );
  });

  Mocha.test(
    "textDocument/hover for definition in the same module",
    async () => {
      const ext = vscode.extensions.getExtension("move.move-analyzer");
      assert.ok(ext);
      ...
      assert.deepStrictEqual(
        (hoverResult.contents as MarkupContent).value,
  ...
  Mocha.test(
    "textDocument/hover for definition in an external module",
    async () => {
      const ext = vscode.extensions.getExtension("move.move-analyzer");
      assert.ok(ext);
  ...
  Mocha.test("textDocument/completion", async () => {
    ...
    const itemsOnColon = await vscode.commands.executeCommand<
      Array<vscode.CompletionItem>
    >("move-analyzer.textDocumentCompletion", colonParams);

    assert.ok(itemsOnColon);

    const keywordsOnColon = itemsOnColon.filter(
    ...
    PRIMITIVE_TYPES.forEach((primitive) => {
      assert.strictEqual(
        isKeywordInCompletionItems(primitive, keywordsOnColon),
        true
      );
    });
  });
