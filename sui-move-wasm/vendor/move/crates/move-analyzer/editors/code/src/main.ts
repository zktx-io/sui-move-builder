...
async function suiMoveCmd(
  context: Readonly<Context>,
  cmd: string
): Promise<void> {
  const version = childProcess.spawnSync(
    context.configuration.suiPath,
    ["--version"],
    { encoding: "utf8" }
  );
  if (version.stdout) {
...
async function testProject(context: Readonly<Context>): Promise<void> {
  const filter = await vscode.window.showInputBox({
    ...
    prompt:
      "Enter filter string to only run tests whose names contain the string" +
...
async function traceProject(context: Readonly<Context>): Promise<void> {
  const filter = await vscode.window.showInputBox({
    ...
    prompt:
      "Enter filter string to only trace tests whose names contain the string" +
  ...
  if (filter !== undefined) {
    const cmd =
      filter.length > 0
        ? `test ${filter} --trace --disassemble`
        : "test --trace --disassemble";
    return suiMoveCmd(context, cmd);
  }
  return Promise.resolve();
}
