...
function version(path: string, args?: readonly string[]): string | null {
  const versionString = childProcess.spawnSync(path, args, {
    encoding: "utf8",
  });
...
function shouldInstall(
  bundledVersionString: string | null,
  bundledVersion: semver.SemVer | null,
  highestVersionString: string | null,
  highestVersion: semver.SemVer | null
): boolean {
  ...
  if (highestVersionString === null || highestVersion === null) {
    log.info(
      `Installing bundled move-analyzer as no existing version found: v${bundledVersion.version}`
    );
    return true;
  }
  if (semver.gt(bundledVersion, highestVersion)) {
    log.info(
      `Installing bundled move-analyzer as the highest version available: v${bundledVersion.version}`
    );
    return true;
  }
  if (
    semver.eq(bundledVersion, highestVersion) &&
    ...
    log.info(
      `Installing bundled move-analyzer equal to the highest version available: v${bundledVersion.version}`
    );
    return true;
  }
  return false;
...
export class Context {
  ...
  registerOnDidChangeConfiguration(): void {
    vscode.workspace.onDidChangeConfiguration(async (event) => {
      const server_path_conf =
        MOVE_CONF_NAME.concat(".").concat(SERVER_PATH_OPT);
      const sui_path_conf = MOVE_CONF_NAME.concat(".").concat(SUI_PATH_OPT);
      const lint_conf = MOVE_CONF_NAME.concat(".").concat(LINT_OPT);
      const auto_imports_conf =
        MOVE_CONF_NAME.concat(".").concat(AUTO_IMPORTS_OPT);
      const force_bundled_conf =
        MOVE_CONF_NAME.concat(".").concat(FORCE_BUNDLED_OPT);
      const type_hints_conf = MOVE_CONF_NAME.concat(".").concat(TYPE_HINTS_OPT);
      const param_hints_conf =
        MOVE_CONF_NAME.concat(".").concat(PARAM_HINTS_OPT);

      const optionsChanged =
        event.affectsConfiguration(lint_conf) ||
      ...
      const pathsChanged =
        event.affectsConfiguration(server_path_conf) ||
  ...
  async installBundledBinary(bundledServerPath: vscode.Uri): Promise<void> {
    ...
    const serverDirExists = await vscode.workspace.fs
      .stat(this.configuration.defaultServerDir)
      .then(
        () => true,
        () => false
      );
    if (serverDirExists) {
      const serverPathExists = await vscode.workspace.fs
        .stat(this.configuration.defaultServerPath)
        .then(
          () => true,
          () => false
        );
      if (serverPathExists) {
        log.info(
          `Deleting existing move-analyzer binary at '${this.configuration.defaultServerPath}'`
        );
        await vscode.workspace.fs.delete(this.configuration.defaultServerPath);
      }
    } else {
      log.info(
        `Creating directory for move-analyzer binary at '${this.configuration.defaultServerDir}'`
      );
      await vscode.workspace.fs.createDirectory(
        this.configuration.defaultServerDir
      );
    }

    log.info(
      `Copying move-analyzer binary to '${this.configuration.defaultServerPath}'`
    );
    await vscode.workspace.fs.copy(
      bundledServerPath,
      this.configuration.defaultServerPath
    );
  }
  ...
  async installServerBinary(
    ...
    if (
      this.configuration.serverPath !==
      this.configuration.defaultServerPath.fsPath
    ) {
      ...
      log.info(
        `Using move-analyzer binary at user-specified path '${this.configuration.serverPath}'`
      );
      return true;
    }
    ...
    if (standaloneVersion !== null) {
      ...
      log.info(
        `Setting v${standaloneVersion.version} of standalone move-analyzer` +
    ...
    if (
      cliVersion !== null &&
      (highestVersion === null || semver.gt(cliVersion, highestVersion))
    ) {
      if (
        bundledVersionString === null ||
        bundledVersion === null ||
        !this.configuration.forceBundled
      ) {
        ...
        log.info(
          `Setting v${cliVersion.version} of CLI move-analyzer` +
            ` installed at '${this.resolvedServerPath}' as the highest one`
        );
      } else {
        log.info(
          `User has requested to use the bundled move-analyzer v${bundledVersion.version}` +
