...
export class Runtime extends EventEmitter {
  ...
  public async start(
    ...
    if (
      ...
      openedFileBaseName === EXT_EVENTS_TRACE_FILE_NAME
    ) {
      ...
      const pkgDirs = extRootContents.filter((dirOrFile) => {
        ...
        return (
          fs.statSync(fullPath).isDirectory() && pkgDirPattern.test(dirOrFile)
        );
      });
      ...
      if (!pkg_name) {
        throw Error(
          `Cannot find package name in manifest file: ${manifest_path}`
        );
      }

      if (
        !openedFilePath.endsWith(MOVE_FILE_EXT) &&
        !openedFilePath.endsWith(TRACE_FILE_EXT)
      ) {
        throw new Error(
          `File extension: ${openedFileExt} is not supported by trace debugger`
        );
      }
      ...
      traceFilePath = path.join(
        pkgRoot,
        "traces",
        traceInfo.replace(/:/g, "_") + TRACE_FILE_EXT
      );

      const disassemblyDir = path.join(
        pkgRoot,
        "build",
        pkg_name,
        "disassembly"
    ...
    if (
      ...
      const newFrame = this.newMoveStackFrame(
        currentEvent.id,
        currentEvent.name,
      ...
      if (path.extname(newFrame.srcFilePath) === BCODE_FILE_EXT) {
        ...
        if (newFrame.bcodeFilePath !== undefined) {
          ...
          throw new Error(
            "Disassembled bytecode file path is not expected to be set for file: " +
      ...
      newFrame.disassemblyModeTriggered =
        disassemblyView && newFrame.bcodeFilePath !== undefined;
  ...
  private stepInternal(
    ...
    if (
      ...
      return {
        kind: ExecutionResultKind.Exception,
        msg: currentEvent.effect.msg,
      };
    } else if (
      currentEvent.type === TraceEventKind.Instruction ||
      ...
      if (
        !eventFrame ||
        !("frames" in eventFrame) ||
        !("globals" in eventFrame)
      ) {
        throw new Error(
          `No active Move call when processing event ${currentEvent.type}`
        );
      }
      ...
      if (currentEvent.type === TraceEventKind.Instruction) {
        ...
        if (stackHeight <= 0) {
          ...
          throw new Error(
            "No frame on the stack when processing Instruction event when stepping"
          );
        }
        ...
        let [sameLine, currentLine] = this.instruction(
          currentFrame,
          currentEvent
        );
        ...
        sameLine =
          ...
          !(
            ...
            (prevEvent.type === TraceEventKind.OpenFrame &&
              (prevEvent.id === INLINED_FRAME_ID_SAME_FILE ||
                prevEvent.id === INLINED_FRAME_ID_DIFFERENT_FILE)) ||
        ...
        if (sameLine) {
          if (
            !next &&
            (currentEvent.kind === TraceInstructionKind.CALL ||
        ...
        if (!currentFrame) {
          throw new Error(
            "No frame to pop when processing `ReplaceInlinedFrame` event"
          );
        }
        ...
        if (currentEvent.isNative) {
          ...
          while (true) {
            ...
            if (executionResult.kind === ExecutionResultKind.TraceEnd) {
              throw new Error(
                "Cannot find CloseFrame event for native function"
              );
            }
            const currentEvent = this.trace.events[this.eventIndex];
        ...
        if (stopAtCloseFrame) {
          ...
          if (framesLength <= 0) {
            throw new Error(
              "No frame to pop at CloseFrame event with ID: " + currentEvent.id
            );
          }
          const currentFrameID = moveCallStack.frames[framesLength - 1].id;
          if (currentFrameID !== currentEvent.id) {
            throw new Error(
              "Frame ID mismatch at CloseFrame event with ID: " +
                currentEvent.id +
                " (current frame ID: " +
                currentFrameID +
                ")"
        ...
        if (effect.type === TraceEffectKind.Write) {
          ...
          if ("globalIndex" in traceLocation) {
            ...
            "frameID" in traceLocation &&
            "localIndex" in traceLocation
          ) {
  ...
  private stepOutInternal(next: boolean): ExecutionResult {
    ...
    if ("frames" in eventFrame && "globals" in eventFrame) {
      ...
      while (true) {
        ...
        if (executionResult.kind === ExecutionResultKind.TraceEnd) {
          throw new Error(
            "Cannot find corresponding CloseFrame event for function: " +
              currentFrame.name
          );
        }
  ...
  private continueInternal(): ExecutionResult {
    while (true) {
      ...
      if (
        executionResult.kind === ExecutionResultKind.TraceEnd ||
    ...
    if (event.type === TraceEventKind.Instruction) {
      ...
      if (eventFrame && "frames" in eventFrame && "globals" in eventFrame) {
        ...
        if (stackHeight <= 0) {
          // this should never happen
          throw new Error(
            "No frame on the stack when processing Instruction event when continuing"
          );
        }
        const currentFrame = moveCallStack.frames[stackHeight - 1];
        ...
        throw new Error(
          "No active Move call when processing Instruction event"
        );
      }
    }
    return false;
  ...
  private instruction(
    ...
    const frameLocalLifetimeEnds = this.trace.localLifetimeEnds.get(
      currentFrame.id
    );
    ...
    if (
      instructionEvent.kind === TraceInstructionKind.CALL ||
  ...
  public setCurrentMoveFileFromPath(filePath: string): string | undefined {
    ...
    if (summaryFrame && !eventFrame) {
      ...
      if (eventFrame && "frames" in eventFrame && "globals" in eventFrame) {
        const moveCallStack = eventFrame as IMoveCallStack;
        if (
          moveCallStack.frames.find((frame) =>
            frame.disassemblyModeTriggered
  ...
  public setCurrentMoveFileFromFrame(frameId: number): string | undefined {
    ...
    if (summaryFrame && !eventFrame) {
      ...
      if (eventFrame && "frames" in eventFrame && "globals" in eventFrame) {
        const moveCallStack = eventFrame as IMoveCallStack;
        const frame = moveCallStack.frames.find(
  ...
  public toggleDisassembly(): void {
    ...
    if (eventFrame && "frames" in eventFrame && "globals" in eventFrame) {
      ...
      moveCallStack.frames.forEach((frame) => {
        if (
          frame.srcFilePath === this.currentMoveFile &&
  ...
  private newMoveStackFrame(
    ...
    if (
      this.trace.events.length <= this.eventIndex + 1 ||
      (this.trace.events[this.eventIndex + 1].type !==
        TraceEventKind.Instruction &&
        this.trace.events[this.eventIndex + 1].type !==
          TraceEventKind.OpenFrame)
    ) {
      throw new Error(
        "Expected an Instruction or OpenFrame event after OpenFrame event"
      );
    }
    return stackFrame;
  }
  ...
  public toString(): string {
    ...
    if (summaryFrame) {
      ...
      for (const summary of summaryFrame.summary) {
        const summaryStr = typeof summary === "string" ? summary : "MoveCall";
    ...
    if (eventFrame) {
      if ("frames" in eventFrame && "globals" in eventFrame) {
        ...
        // an empty one.
        const moveCallStack: IMoveCallStack = {
          frames: [],
          globals: new Map(),
        };
  ...
  public moveCallStacktoString(
    ...
    for (const frame of moveCallStack.frames) {
      ...
      const line = frame.disassemblyModeTriggered
        ? frame.bcodeLine
        : frame.srcLine;
      res +=
        tabs +
      ...
      for (let i = 0; i < frame.locals.length; i++) {
        ...
        for (let j = 0; j < frame.locals[i].length; j++) {
          ...
          if (
            local &&
            (frame.disassemblyView || !isGeneratedLocal(local.info))
          ) {
...
function localWrite(
  ...
  if (!type) {
    throw new Error(
      "Cannot find type for local variable at index: " +
        localIndex +
        " in function: " +
        frame.name
    );
  ...
  if (!localInfo) {
    throw new Error(
      "Cannot find local variable at index: " +
        localIndex +
        " in function: " +
        frame.name
    );
  ...
  if (scopesCount <= 0) {
    throw new Error(
      "There should be at least one variable scope in function" + frame.name
    );
  }
  ...
  if (existingVarScope >= 0) {
    ...
    frame.locals[existingVarScope + 1][localIndex] = {
      info: localInfo,
      value,
      type,
      frameIdx,
