...
interface JSONSrcEnumSourceMapEntry {
  ...
  type_parameters: [string, JSONSrcDefinitionLocation][];
  variants: [
    [string, JSONSrcDefinitionLocation],
    JSONSrcDefinitionLocation[],
  ][];
}
  ...
  for (const debugInfo of debugInfosMap.values()) {
    ...
    if (debugInfoLines && fileInfo) {
      for (let i = 0; i < fileInfo.lines.length; i++) {
        if (!debugInfoLines.has(i + 1)) {
          // allDebugInfoLines is 1-based
...
function readDebugInfo(
  ...
  const debugInfoJSON: JSONSrcRootObject = JSON.parse(
    fs.readFileSync(debugInfoPath, "utf8")
  );

  let fileHash = Buffer.from(
    debugInfoJSON.definition_location.file_hash
  ).toString("base64");
  let fileInfo = filesMap.get(fileHash);
  if (!fileInfo) {
    if (failOnNoSourceFile) {
      throw new Error(
        "Could not find file with hash: " +
          fileHash +
  ...
  if (
    debugInfoJSON.from_file_path !== undefined &&
  ...
  const addr =
    pkgVersionID && debugInfoModAddr === 0 ? pkgVersionID : debugInfoAddrStr;
  ...
  for (const funEntry of Object.values(functionMap)) {
    ...
    for (const [pc, defLocation] of Object.entries(funEntry.code_map)) {
      ...
      const defLocFileHash = Buffer.from(defLocation.file_hash).toString(
        "base64"
      );
      ...
      const currentStartLoc = byteOffsetToLineColumn(
        fileInfo,
        defLocation.start
      );
      ...
      const debugInfoLines =
        debugInfoLinesMap.get(defLocFileHash) ?? new Set<number>();
...
function prePopulateDebugInfoLines(
  ...
  addLinesForLocation(
    sourceMapJSON.definition_location,
    fileInfo,
    debugInfoLines
  );
  ...
  for (const structEntry of Object.values(structMap)) {
    addLinesForLocation(
      structEntry.definition_location,
      fileInfo,
      debugInfoLines
    );
