 ...
 */
interface CustomizedStackTraceResponse
  extends DebugProtocol.StackTraceResponse {
  body: {
...
export class MoveDebugSession extends LoggingDebugSession {
  ...
  public constructor() {
    ...
    this.runtime.on(RuntimeEvents.stopOnLineBreakpoint, () => {
      this.sendEvent(
        new StoppedEvent("breakpoint", MoveDebugSession.THREAD_ID)
      );
    });
    this.runtime.on(RuntimeEvents.stopOnException, (msg) => {
      this.sendEvent(
        new StoppedEvent("exception", MoveDebugSession.THREAD_ID, msg)
      );
    });
  ...
  protected dispatchRequest(request: DebugProtocol.Request): void {
    if (request.command === "toggleDisassembly") {
      ...
      this.sendEvent(
        new StoppedEvent("toggle disassembly", MoveDebugSession.THREAD_ID)
      );
    } else if (request.command === "toggleSource") {
      this.runtime.toggleSource();
      this.sendEvent(
        new StoppedEvent("toggle source", MoveDebugSession.THREAD_ID)
      );
    } else if (request.command === "fileChanged") {
      const newFilePath = String(request.arguments);
      const changedFilePath =
        this.runtime.setCurrentMoveFileFromPath(newFilePath);
  ...
  protected async launchRequest(
    ...
    logger.log(
      `Launching trace viewer for file: ${args.source} and trace: ${args.traceInfo}`
    );
  ...
  protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
    response.body = {
      threads: [new Thread(MoveDebugSession.THREAD_ID, "Main Thread")],
    };
    this.sendResponse(response);
  }
    ...
    try {
      ...
      if (eventFrame) {
        if ("frames" in eventFrame && "globals" in eventFrame) {
          ...
          stackFrames.push(
            ...moveCallStack.frames.map((frame) => {
          ...
          if (stack_height > 0) {
            optimizedLines = moveCallStack.frames[stack_height - 1]
  ...
  private getScopes(frameID: number): DebugProtocol.Scope[] {
    ...
    if ("frames" in eventFrame && "globals" in eventFrame) {
      ...
      if (!frame) {
        throw new Error(
          `No frame found for id: ${frameID} when getting scopes`
        );
      }
      if (frame.locals.length > 0) {
        for (let i = frame.locals.length - 1; i > 0; i--) {
          const shadowedScopeReference = this.variableHandles.create({
            locals: frame.locals[i],
          });
          const shadowedScope = new Scope(
            `shadowed(${i}): ${frame.name}`,
            shadowedScopeReference,
            false
          );
      ...
      const localScopeReference = this.variableHandles.create({
        locals: frame.locals[0],
      });
  ...
  protected scopesRequest(
    ...
    try {
      ...
      const changedFile = this.runtime.setCurrentMoveFileFromFrame(
        args.frameId
      );
      logger.log(
        "Current Move file changed to " +
          changedFile +
          " for frame id " +
          args.frameId
      );
  ...
  private convertMoveRefValue(
    ...
    if ("globalIndex" in indexedLoc.loc) {
      ...
      throw new Error(
        "Invalid runtime location when comverting Move call ref value"
      );
    }
  }
  ...
  private convertRuntimeVariables(
    ...
    runtimeVariables.forEach((v) => {
      if (v) {
        const varName = disassemblyView ? v.info.internalName : v.info.name;
  ...
  protected variablesRequest(
    ...
    try {
      ...
      if (eventFrame) {
        const variableHandle = this.variableHandles.get(
  ...
  protected nextRequest(
    ...
    let terminate = false;
    try {
      const executionResult = this.runtime.step(
        /* next */ true,
  ...
  protected setBreakPointsRequest(
    ...
    try {
      ...
      if (args.breakpoints && args.source.path) {
        ...
        const validatedBreakpoints = this.runtime.setLineBreakpoints(
          args.source.path,
          breakpointLines
        );
        for (let i = 0; i < breakpointLines.length; i++) {
          finalBreakpoints.push(
            new Breakpoint(validatedBreakpoints[i], breakpointLines[i])
          );
        }
      }
      response.body = {
