...
type JSONTraceLocation =
  | JSONTraceLocalLocation
  | JSONTraceIndexedLocation
  | JSONTraceGlobalLocation;
  ...
  for (const event of traceJSON.events) {
    if (event.OpenFrame) {
      ...
      for (let i = 0; i < frame.parameters.length; i++) {
        ...
        if (value) {
          const runtimeValue: RuntimeValueType =
            "RuntimeValue" in value
      ...
      if (!srcFunEntry) {
        throw new Error(
          "Cannot find function entry in debug info for function " +
            frame.function_name +
            " in module " +
            modInfo.name +
            " in package " +
      ...
      const instSrcFileLoc =
        event.Instruction.pc >= srcPCLocs.length
      ...
      if (frameInfo.bcodeFunEntry?.pcLocs) {
        ...
        instBcodeFileLoc =
          event.Instruction.pc >= bcodePCLocs.length
      ...
      if (
        frameInfo.ID === INLINED_FRAME_ID_SAME_FILE ||
        ...
        for (let i = frameInfoStack.length - 1; i >= 0; i--) {
          ...
          if (
            currentFrame.ID !== INLINED_FRAME_ID_SAME_FILE &&
      ...
      if (effect.Write || effect.Read || effect.DataLoad) {
        ...
        if (effect.Write) {
          ...
          const value =
            "RuntimeValue" in effect.Write.root_value_after_write
              ? traceRuntimeValueFromJSON(
                  effect.Write.root_value_after_write.RuntimeValue.value
                )
              : "globalIndex" in runtimeLoc.loc
                ? // We are writing to a global value here. Global values are
                  ...
                  derefTraceRefValueFromJSON(
                    effect.Write.root_value_after_write
                  )
      ...
      if (typeof external === "string") {
        ...
        const summary: ExtEventSummary[] = external.Summary.events.map((s) => {
          if (
            ...
            "MoveCall" in s &&
            s.MoveCall &&
            typeof s.MoveCall === "object" &&
            "pkg" in s.MoveCall &&
            "module" in s.MoveCall &&
            "function" in s.MoveCall
          ) {
            ...
            throw new Error(
              "Unexpected external summary event: " + JSON.stringify(s)
            );
          }
        });
        events.push({
        ...
        for (const v of external.ExternalEvent.values) {
          if (v.Single) {
            ...
            localsTypes.push(
              JSONTraceTypeToString(type_.type_, type_.ref_type)
            );
            ...
            localsTypes.push(
              `vector<${JSONTraceTypeToString(type_.type_, type_.ref_type)}>`
            );

            localsNames.push(v.Vector.name);
            localsValues.push(
              v.Vector.value.map((v) => {
...
function processInstructionIfMacro(
  ...
  if (instSrcFileLoc.fileHash !== frameInfo.srcFileHash) {
    ...
    // frames on the stack.
    if (
      frameInfoStack.length > 1 &&
      frameInfoStack[frameInfoStack.length - 2].srcFileHash ===
      ...
      if (!sourceMap) {
        throw new Error(
          "Cannot find source map for file with hash: " +
            instSrcFileLoc.fileHash +
 ...
 */
function JSONTraceTypeToString(
  baseType: JSONBaseType,
  refType?: JSONTraceRefType
): string {
  const refPrefix =
    refType === JSONTraceRefType.Mut
      ? "&mut "
  ...
  if (typeof baseType === "string") {
    ...
    return JSONCompoundTypeToString(
      refPrefix,
...
function processJSONLocation(
  ...
  if ("Local" in traceLocation) {
    ...
    return processJSONLocation(
      traceLocation.Indexed[0],
      indexPath,
      localLifetimeEnds
    );
  } else {
 ...
 * @throws Error with a descriptive error message if conversion has failed.
 */
function derefTraceRefValueFromJSON(
  value: JSONTraceRefValue
): RuntimeValueType {
  if ("MutRef" in value) {
 ...
 */
function traceRuntimeValueFromJSON(
  moveValue: JSONTraceMoveValue
): RuntimeValueType {
  if (
    moveValue.type === "U8" ||
    moveValue.type === "U16" ||
    ...
    (moveValue.type === "Struct" || moveValue.type === "Variant") &&
    typeof moveValue.value === "object" &&
    "type_" in moveValue.value
  ) {
    const fields: [string, RuntimeValueType][] = moveValue.value.fields.map(
      ([key, value]) => [key, traceRuntimeValueFromJSON(value)]
    );
    ...
    case TraceEventKind.ExternalSummary:
      ...
      for (const c of event.summary) {
        if (
          ...
          events += c.pkg + "::" + c.module + "::" + c.function;
        } else {
          events += c.toString();
        }
        events += "\n";
