...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.debug.registerDebugConfigurationProvider(DEBUGGER_TYPE, provider)
  );
  context.subscriptions.push(
          ...
          if (optimized_lines && optimized_lines.length > 0) {
            ...
            if (stackFrame && stackFrame.source) {
              ...
              optimized_lines.forEach((lineNumber: number) => {
                ...
                if (
                  lineText.length !== 0 && // ignore empty lines
                  !lineText.startsWith("const") && // ignore constant declarations (not in the source map)
                  !lineText.startsWith("}")
                ) {
                  // ignore closing braces with nothing else on the same line
                  const decoration = {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("move.toggleDisassembly", () => {
      const session = vscode.debug.activeDebugSession;
      if (session) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("move.toggleSource", () => {
      const session = vscode.debug.activeDebugSession;
      if (session) {
  ...
  context.subscriptions.push(
    vscode.workspace.registerTextDocumentContentProvider(
      TRACE_FILE_URI_SCHEME,
      trace_content_provider
    )
  );
  context.subscriptions.push(
  ...
  vscode.workspace.onDidOpenTextDocument(async (doc) => {
    if (doc.uri.scheme === "file" && doc.uri.fsPath.endsWith(".json.zst")) {
      ...
      const mtraceUri = vscode.Uri.parse(
        `${TRACE_FILE_URI_SCHEME}:${doc.uri.fsPath}`
      );
...
class MoveTraceViewProvider implements vscode.CustomReadonlyEditorProvider {
  ...
  async resolveCustomEditor(
    ...
    try {
      const traceContent = trimTraceFileContent(
        await decompressTraceFile(document.uri.fsPath)
      );
  ...
  async resolveDebugConfiguration(
    ...
    if (!config.request && !config.name) {
      try {
        ...
        if (
          editor &&
          (editor.document.languageId === MOVE_LANGUAGE_ID ||
          ...
          if (traceViewUri) {
            ...
            const traceFilesPattern = new vscode.RelativePattern(
              folder.uri.fsPath,
              "**/*.json.zst"
            );
            const traceFilePaths =
              await vscode.workspace.findFiles(traceFilesPattern);
            if (traceFilePaths.length === 0) {
              return vscode.window
                .showErrorMessage("No trace files found in the workspace.")
                .then((_) => {
                  return undefined;
                });
            }
            const tracePath =
              traceFilePaths.length === 1
                ? traceFilePaths[0].fsPath
                : await pickTraceFileToDebug(
                    folder.uri.fsPath,
                    traceFilePaths.map((file) => file.fsPath)
                  );

            if (!tracePath) {
              return vscode.window
                .showErrorMessage("No trace file selected.")
                .then((_) => {
                  return undefined;
                });
            }
...
async function findTraceInfo(
  ...
  if (openedFilePath.endsWith(TRACE_FILE_EXT)) {
    ...
    if (openedFileExt !== MOVE_FILE_EXT) {
      throw new Error(
        "Unsupported file extension to start debugging " +
          `'${openedFileExt}'` +
  ...
  if (!fun) {
    throw new Error(
      `No function to be trace - debugged selected from\n` +
 ...
 */
function findTracedFunctionsFromPath(
  pkgRoot: string,
  pkgModules: string[]
): string[] {
  ...
  pkgModules.forEach((module) => {
    ...
    const prefixFiles = filePaths.filter((filePath) =>
 ...
 */
async function getTracedFunctionInfo(
  traceFilePath: string
): Promise<TracedFunctionInfo> {
  ...
  if (!pkgAddrStrInTrace) {
    throw new Error(
      `No package address for the initial frame in first trace event: '${firstEvent}'`
    );
  }
  const pkgAddrInTrace = parseInt(pkgAddrStrInTrace);
  if (isNaN(pkgAddrInTrace)) {
    throw new Error(
      "Cannot parse package address " +
        pkgAddrStrInTrace +
  ...
  if (!moduleInTrace) {
    throw new Error(
      `No module name for the initial frame in first trace event: '${firstEvent}'`
    );
  }
  const functionInTrace = frame?.function_name;
  if (!functionInTrace) {
    throw new Error(
      `No function name for the initial frame in first trace event: '${firstEvent}'`
    );
  }
  return {
 ...
 *
 */
async function constructTraceInfo(
  tracePath: string
): Promise<string | undefined> {
  if (
    tracePath.endsWith(TRACE_FILE_EXT) &&
  ...
  if (tracedFileBaseName === EXT_EVENTS_TRACE_FILE_NAME) {
    ...
    // `<package>__<module>__<function_name>.json`
    const fileBaseNameSuffix =
      "__" + tracedFunctionInfo.module + "__" + tracedFunctionInfo.function;
    ...
    const pkgName = tracedFileBaseName.substring(
      0,
      tracedFileBaseName.length - fileBaseNameSuffix.length
    );
    return (
      pkgName +
      "::" +
...
function getTraceFiles(pkgRoot: string): string[] {
  ...
  try {
    ...
    throw new Error(
      `Error accessing 'traces' directory for package at '${pkgRoot}'`
    );
  }
...
async function pickFunctionToDebug(
  ...
  return selectedFunction
    ? selectedFunction.pkg + "::" + selectedFunction.label
    : undefined;
}
...
function traceViewTabUri(): vscode.Uri | undefined {
  ...
  if (input instanceof vscode.TabInputCustom) {
    if (
      input.viewType === TRACE_CUSTOM_EDITOR_ID &&
