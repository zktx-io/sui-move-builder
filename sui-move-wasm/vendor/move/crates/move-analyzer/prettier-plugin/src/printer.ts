...
const { hardline, indent, join, line, softline, group, ifBreak } =
...
export function print(path: AstPath, options: ParserOptions, print: printFn) {
  ...
  switch (node.type) {
    ...
    case "module_body":
      if (node.children.length == 2) {
        ...
        return [
          ...
          indent([
            [hardline, hardline],
            join([hardline, hardline], path.map(print, "namedChildren")),
          ]),
          hardline,
          "}",
        ];
    ...
    case "function_definition":
      ...
      return [
        node.namedChild(0).type === "visibility_modifier"
          ? [path.call(print, "namedChildren", 0), " "]
          : "",
        is_entry ? "entry " : "",
        "fun ",
    ...
    // TODO: do macros
    case "ability_decls":
      return [" has ", join(", ", path.map(print, "namedChildren"))];
...
function breakable_comma_separated_list(
  path: AstPath,
  node: SyntaxNode,
  start: string,
  end: string,
